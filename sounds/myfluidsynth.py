...
fluid_synth_get_cc = cfunc('fluid_synth_get_cc', c_int, ('synth', c_void_p, 1), ('chan', c_int, 1), ('num', c_int, 1),
                           ('pval ', c_int, 1))

fluid_synth_get_channel_preset = cfunc('fluid_synth_get_channel_preset', c_void_p, ('synth', c_void_p, 1),
                                       ('chan', c_int, 1))

#fluid_sfont_iteration_start = cfunc('fluid_sfont_iteration_start', c_void_p,('sfont', c_void_p, 1))

#fluid_sfont_iteration_next = cfunc('fluid_sfont_iteration_next', c_void_p,('sfont', c_void_p, 1))

#fluid_preset_get_name = cfunc('fluid_preset_get_name', c_char, ('preset', c_void_p, 1))
...
class Synth:
    ...
    def __init__(self, gain=0.2, samplerate=44100):
        """Create a new synthesizer object to control sounds generation.
    ...
    def sfload(self, filename, update_midi_preset=0):
        ...
        self.sfid = fluid_synth_sfload(self.synth, str_binary(filename), update_midi_preset)
        return self.sfid

    def sfunload(self, update_midi_preset=0):
        """Unload a SoundFont and free memory it used."""
        if hasattr(self, 'sfid'):
            return fluid_synth_sfunload(self.synth, self.sfid, update_midi_preset)
        pass

    def program_select(self, chan, bank, preset):
        """Select a program."""
        return fluid_synth_program_select(self.synth, chan, self.sfid, bank, preset)

    def sleep(self, seconds):
        if hasattr(self, 'wav'):
            samples = self.raw_audio_string(self.fs.get_samples(
                int(seconds * 44100)))
    ...
    # add new
    def fluid_synth_get_cc(self, channel, num, pval):
        return fluid_synth_get_cc(self.synth, channel, num, pval)

    # add new
    def fluid_synth_get_channel_preset(self, channel):
        # self.synth.argtypes = [POINTER(fluid_synth_t), c_int]
        # self.synth.restype = POINTER(fluid_preset_t)
        # print(self.synth.argtypes)
        return fluid_synth_get_channel_preset(self.synth, channel)

    # add new
    def fluid_synth_get_presets(self):
        pass
        '''fluid_sfont_iteration_start(self.sfid)
        preset = None
        while (preset == fluid_sfont_iteration_next(self.sf)) and preset != None:
            print(fluid_preset_get_name(preset))'''
