...
node = ("C","C#","D","Eb","E","F","F#","G","G#","A","Bb","B")
fretNum = 18
...
def scale():
    ...
    drawBoard(noteStr)
    sd.speed=0.4
    sd.playNotes(list(noteStr.values()))
def chord():
    global noteStr
    noteStr=c.getChord(rootNoteCombo.get(),proCombo.get(),intervalCombo.get())
    resultLabel.config(text=list(noteStr.values()))
    drawBoard(noteStr)
    print(list(noteStr.values()))
    sd.playChord(list(noteStr.values()))
def clear():
...
def initNote():
    for i in range(6):
        ...
        for j in range(fretNum):
            labelTmp = Button(frerBoard,compound = 'center')
            labelTmp.bind("<Button-1>",clickBtn)
...
def drawBoardLabel(note,noteType,noteName):
    Style().configure("ROOT.TButton", foreground='white')
    Style().configure("NORMAL.TButton", foreground='black')
    if fretLabelType == 0:
        if noteType==0:        
            note.config(text = '')
        elif noteType==1:        
            note.config(text = noteStr.get(noteName),style = 'NORMAL.TButton')
        elif noteType==2:        
            note.config(text = noteStr.get(noteName),style = 'ROOT.TButton')
        else:
        ...
        if noteType==0:        
            ...
            note.config(text = noteName,style = 'NORMAL.TButton')
        elif noteType==2:        
            note.config(text = '1',style = 'ROOT.TButton')
        else:
            print('error noteType')
    else:
